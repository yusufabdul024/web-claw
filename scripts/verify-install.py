...
REQUIRED_REFERENCE_FILES = [
    ...
    "references/client-collaboration.md",
    "references/competitor-analysis.md",
    "references/premium-experience-standard.md",
...
def check_skill_frontmatter(root: Path) -> Check:
    ...
    if not desc_match:
        ...
        block_styles = {"|", "|-", "|+", ">", ">-", ">+"}
        if desc_value in block_styles:
            lines = block.splitlines()
            desc_line_index = next(
                (i for i, line in enumerate(lines) if re.match(r"^description\s*:", line)),
                None,
            )
            has_indented_body = (
                desc_line_index is not None
                and any(line.startswith((" ", "\t")) and line.strip() for line in lines[desc_line_index + 1 :])
            )
            if not has_indented_body:
                c.fail("frontmatter `description:` block scalar has no indented body")
        elif not desc_value:
            c.fail("frontmatter `description:` is empty")
        elif len(desc_value) > 280:
            c.fail(
                "frontmatter `description:` is too long on one line; use a short YAML block scalar so Codex can roster the skill"
            )
    return c
...
_RUNTIME_GENERATED = {
    "memory.md",       # produced at BRIEF:INTERVIEW inside <project>/
    "client-brief.md",         # produced inside <project>/brief/
    "brief/client-brief.md",   # produced inside <project>/brief/
    "plan.md",         # produced after BUILD:PLAN
